...
/// <summary>
/// Direct P/Invoke surface for <c>headsetcontrol_c.h</c>.
/// </summary>
/// <remarks>
/// <para>
/// All entry points are bound via <see cref="LibraryImportAttribute"/> so that
/// the source-generated marshalling code can be trimmed and AOT-compiled.
/// </para>
/// <para>
/// String returns from the native library are <c>const char*</c> pointers that
/// remain valid for the lifetime of the corresponding native object — they are
/// marshalled as <see cref="IntPtr"/> here and decoded by callers (see
/// <see cref="NativeStringMarshaller"/>) rather than copied automatically, to
/// avoid the source generator emitting <c>free()</c> calls on memory the
/// library still owns.
/// </para>
/// <para>
/// The assembly carries <c>[DefaultDllImportSearchPaths(SafeDirectories)]</c>,
/// which restricts the OS loader to safe directories — the actual per-RID
/// resolution is handled by <see cref="NativeLibraryLoader"/>.
/// </para>
/// </remarks>
